package com.youlan.common.core.sensitive.helper;
...
public class SensitizeHelper {
     ...
     *
     * @param str           字符串
     * @param sensitiveType 脱敏类型;可以脱敏：用户id、中文名、身份证号、座机号、手机号、地址、电子邮件、密码
     * @return 脱敏之后的字符串
     * @author dazer and neusoft and qiaomu
     * @since 5.6.2
     */
    public static String desensitized(CharSequence str, SensitiveType sensitiveType) {
     ...
     * 自定义脱敏
     *
     * @param customStr       需要脱敏的内容
     * @param prefixNoMaskLen 左侧需要保留明文的长度
     * @param suffixNoMaskLen 右侧需要保留明文的长度
     * @param maskStr         自定义脱敏字符
     * @return 脱敏后的内容
     */
    public static String customMask(String customStr, int prefixNoMaskLen, int suffixNoMaskLen, String maskStr) {
        if (StrUtil.isBlank(customStr)) {
            return StrUtil.EMPTY;
        }
        StringBuilder sensitiveStr = new StringBuilder();
        for (int i = 0, n = customStr.length(); i < n; i++) {
            if (i < prefixNoMaskLen) {
                sensitiveStr.append(customStr.charAt(i));
                continue;
            }
